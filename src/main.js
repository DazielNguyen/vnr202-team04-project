...
import canVuongImage from '../assets/phongtrao/can-vuong.jpg';
...
import posterOne from '../assets/poster-01-new.svg?url';
import posterTwo from '../assets/poster-02-new.svg?url';
...
function shell(content){return `
  ...
  <aside class="rail" aria-label="Tiến trình"><span class="rail-line"><i></i></span>${Array.from({length:11},(_,i)=>`<a href="#scene-${i+1}" aria-label="Cảnh ${i+1}" class="${i===0?'active':''}">${String(i+1).padStart(2,'0')}</a>`).join('')}</aside>
  <nav class="page-nav" aria-label="Chuyển trang"><button class="page-prev" type="button" aria-label="Trang trước">← <span>TRƯỚC</span></button><b>01 / 11</b><button class="page-next" type="button" aria-label="Trang sau"><span>SAU</span> →</button></nav>
...
function home(){return shell(`
 ...
 <section class="scene video-showcase" id="scene-2" data-scene="2"><div class="scene-inner">
  <header class="media-page-heading"><p class="chapter">GIỚI THIỆU · CLIP TRÌNH BÀY</p><h2>Xem trước<br><em>hành trình 1930.</em></h2></header>
  <div class="video-stage"><video id="presentation-video" controls preload="metadata" playsinline poster="${internationalContextImage}"><source src="${presentationVideo}" type="video/mp4">Trình duyệt của bạn không hỗ trợ phát video.</video><button class="media-fullscreen" type="button" data-fullscreen="#presentation-video" aria-label="Mở clip toàn màn hình">⛶ <span>TOÀN MÀN HÌNH</span></button></div>
 </div></section>
 <section class="scene crisis" id="scene-3" data-scene="3"><div class="scene-inner">
 ...
 <section class="scene context" id="scene-5" data-scene="5"><div class="scene-inner">
 ...
 <section class="scene preparation" id="scene-6" data-scene="6"><div class="scene-inner">
 ...
 <section class="scene paradox" id="scene-7" data-scene="7"><div class="scene-inner">
 ...
 <section class="scene conference" id="scene-8" data-scene="8"><div class="scene-inner">
 ...
 <section class="scene platform" id="scene-9" data-scene="9"><div class="scene-inner">
 ...
 <section class="scene turning" id="scene-10" data-scene="10"><div class="scene-inner">
  ...
  <div class="finale-footer"><div class="finale-evidence">${label('group')} ${sourceButton('turning')}</div><div class="final-actions"><button id="quiz-final">Kiểm tra hiểu biết</button><a href="${textbookUrl}" target="_blank" rel="noopener">Mở giáo trình gốc</a><a href="#scene-6">Xem lại dòng thời gian</a></div></div><p class="transparency">AI hỗ trợ tổ chức nội dung, thiết kế và phát triển. Dữ kiện lịch sử được đối chiếu với giáo trình gốc.</p>
 </div></section>
 <section class="scene creative-showcase" id="scene-11" data-scene="11"><div class="scene-inner">
  <header class="media-page-heading"><p class="chapter">SẢN PHẨM SÁNG TẠO</p><h2>Hai poster<br><em>từ hành trình học tập.</em></h2><p>Hai thiết kế trực quan hóa thông điệp của bài thuyết trình. Chọn biểu tượng toàn màn hình để xem từng poster ở kích thước đầy đủ.</p></header>
  <div class="poster-grid">${[{id:'poster-01',src:posterOne,label:'POSTER 01'},{id:'poster-02',src:posterTwo,label:'POSTER 02'}].map(p=>`<figure class="poster-card" id="${p.id}"><img src="${p.src}" alt="${p.label} — sản phẩm sáng tạo của nhóm" loading="lazy"><figcaption>${p.label}</figcaption><button class="media-fullscreen" type="button" data-fullscreen="#${p.id}" aria-label="Mở ${p.label} toàn màn hình">⛶ <span>TOÀN MÀN HÌNH</span></button></figure>`).join('')}</div>
 </div></section>`)}
...
function bind(){
 ...
 document.querySelectorAll('[data-fullscreen]').forEach(button=>button.addEventListener('click',()=>{const target=document.querySelector(button.dataset.fullscreen);if(!target)return;if(document.fullscreenElement===target){document.exitFullscreen();return}target.requestFullscreen?.().catch(()=>{})}));
...
function setupNavigation(){
 ...
 const chapterNames=['DẪN NHẬP','CLIP GIỚI THIỆU','KHỦNG HOẢNG','BỐI CẢNH QUỐC TẾ','BỐI CẢNH TRONG NƯỚC','QUÁ TRÌNH CHUẨN BỊ','NHU CẦU THỐNG NHẤT','HỘI NGHỊ HỢP NHẤT','CƯƠNG LĨNH','BƯỚC NGOẶT','SẢN PHẨM SÁNG TẠO'];
