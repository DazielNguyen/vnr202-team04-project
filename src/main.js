...
function shell(content){return `
  ...
  <aside class="rail" aria-label="Tiến trình"><span class="rail-line"><i></i></span>${Array.from({length:8},(_,i)=>`<a href="#scene-${i+1}" aria-label="Cảnh ${i+1}" class="${i===0?'active':''}">${String(i+1).padStart(2,'0')}</a>`).join('')}</aside>
  <nav class="page-nav" aria-label="Chuyển trang"><button class="page-prev" type="button" aria-label="Trang trước">← <span>TRƯỚC</span></button><b>01 / 08</b><button class="page-next" type="button" aria-label="Trang sau"><span>SAU</span> →</button></nav>
...
function home(){return shell(`
 ...
 <section class="scene preparation" id="scene-4" data-scene="4"><div class="scene-inner">
  <div class="preparation-intro" data-preparation-intro><div class="section-head"><p class="chapter">03 · QUÁ TRÌNH CHUẨN BỊ</p><h2>Năm bước chuẩn bị<br><em>cho một bước ngoặt.</em></h2><p class="preparation-summary">Từ hành trình tìm đường cứu nước đến sự hình thành lực lượng và tổ chức cách mạng.</p></div>
 ...
 <section class="scene conference" id="scene-6" data-scene="6"><div class="scene-inner">
  <header class="conference-heading"><div><p class="chapter">05 · HỘI NGHỊ HỢP NHẤT</p><h2>Từ nhu cầu lịch sử<br>đến <em>quyết định.</em></h2></div><p>Hội nghị biến yêu cầu thống nhất các tổ chức cộng sản thành một quyết định chính trị và tổ chức cụ thể.</p></header>
 ...
 <section class="scene platform" id="scene-7" data-scene="7"><div class="scene-inner">
  <header class="platform-heading"><div><p class="chapter">06 · ĐƯỜNG LỐI</p><h2>Một tổ chức thống nhất<br>cần một <em>Cương lĩnh.</em></h2></div><p>Cương lĩnh đầu tiên trả lời sáu câu hỏi nền tảng: đi đâu, làm gì, vì ai, bằng cách nào và dưới sự lãnh đạo của ai.</p></header>
 ...
 <section class="scene turning" id="scene-8" data-scene="8"><div class="scene-inner">
  <header class="finale-hero"><div class="finale-year"><small>BƯỚC NGOẶT</small><strong>1930</strong></div><div class="finale-question"><p class="chapter">07 · KẾT LUẬN</p><h2>Thay đổi điều gì<br><em>về chất?</em></h2><p>Không chỉ thêm một tổ chức mới, năm 1930 tạo ra một hệ thống lãnh đạo cách mạng thống nhất.</p></div></header>
  ...
  <div class="finale-footer"><div class="finale-evidence">${label('group')} ${sourceButton('turning')}</div><div class="final-actions"><button id="quiz-final">Kiểm tra hiểu biết</button><a href="${textbookUrl}" target="_blank" rel="noopener">Mở giáo trình gốc</a><a href="#scene-4">Xem lại dòng thời gian</a></div></div><p class="transparency">AI hỗ trợ tổ chức nội dung, thiết kế và phát triển. Dữ kiện lịch sử được đối chiếu với giáo trình gốc.</p>
 </div></section>`)}
...
function setupNavigation(){
 ...
 const chapterNames=['DẪN NHẬP','KHỦNG HOẢNG','ĐIỀU KIỆN XÃ HỘI','QUÁ TRÌNH CHUẨN BỊ','NHU CẦU THỐNG NHẤT','HỘI NGHỊ HỢP NHẤT','CƯƠNG LĨNH','BƯỚC NGOẶT'];
 const pages=[...scenes.slice(0,3),...(intro?[intro]:[]),...stages,...scenes.slice(4)];
 let current=0;
 const activate=index=>{if(index<0)return;current=index;const target=pages[index],stageIndex=stages.indexOf(target),sceneIndex=target===intro||stageIndex>=0?3:scenes.indexOf(target);scenes[sceneIndex]?.classList.add('scene-active');if(target===intro)intro.classList.add('stage-active');if(stageIndex>=0)target.classList.add('stage-active');document.querySelectorAll('.rail a').forEach((a,i)=>a.classList.toggle('active',i===sceneIndex));const progress=document.querySelector('.rail i');if(progress)progress.style.height=`${sceneIndex/7*100}%`;if(status)status.textContent=`${String(sceneIndex).padStart(2,'0')} · ${stageIndex>=0?`${chapterNames[sceneIndex]} — ${stages[stageIndex].dataset.stageTitle}`:chapterNames[sceneIndex]}`;if(counter)counter.textContent=stageIndex>=0?`04 · ${String(stageIndex+1).padStart(2,'0')}/08`:target===intro?'04 · TỔNG QUAN':`${String(sceneIndex+1).padStart(2,'0')} / 08`;if(prev)prev.disabled=index===0;if(next)next.disabled=index===pages.length-1};
